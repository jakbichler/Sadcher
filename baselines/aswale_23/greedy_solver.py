...
def greedy_task_allocation(Q, R, T_execution, T_travel):
    # Initialize the ready task list with tasks that have no predecessors
    ready_tasks = set()
    for task in range(1, n_tasks + 1):
        if not any(k == task for _, k in precedence_constraints):
            ready_tasks.add(task)

    while not all_tasks_satisfied(X, R):

        # Filter max contribution pairs to only include those meeting precedence constraints
        max_contribution_pairs = [
            (robot, task) for robot, task in find_max_contribution_pairs(Q, R, ready_tasks)
            if check_precedence_constraints(task, Y_max, precedence_constraints)
        ]

        if not max_contribution_pairs:
            raise ValueError("No valid task assignments available that satisfy precedence constraints.")
        ...
        update_ready_tasks(selected_task, ready_tasks, Y_max, precedence_constraints, T_execution)

    return X

def update_ready_tasks(completed_task, ready_tasks, Y_max, precedence_constraints, T_execution):
    # Add tasks to the ready list if all their predecessors are scheduled and finished
    for j, k in precedence_constraints:
        if j == completed_task and k not in ready_tasks:
            # Get all predecessors of task k
            predecessors = [pred for pred, succ in precedence_constraints if succ == k]

            # Check if all predecessors are completed
            if all(Y_max[pred] != -1 for pred in predecessors):
                # Set Y_max for task k as greater than or equal to the max finish time of all predecessors
                Y_lower_bound[k] = max(Y_max[pred] + T_execution[pred] for pred in predecessors)
...
def all_tasks_satisfied(X,R):
    ...
    """
    for task in range(1, n_tasks + 1):  
        if not task_satisfied(X, R, task):
            return False

    return True


def check_precedence_constraints(task, Y_max, precedence_constraints):
    for j, k in precedence_constraints:
        if k == task:
            if Y_max[j] == -1:
                return False  # Predecessor task not scheduled yet
            finish_time_j = Y_max[j] + T_execution[j]
            if finish_time_j > Y_max[task] and Y_max[task] != -1:
                return False
    return True

def find_max_contribution_pairs(Q, R, ready_tasks):
    max_contribution = 0
    max_contribution_pairs = []
...
def select_earliest_pair(pairs, Y, T_execution, T_travel):
    ...
    for pair in pairs:
        ...
        arrival_time = max(arrival_time, Y_lower_bound[task])
...
if __name__ == "__main__":
    ...
    Y_max = np.full(n_tasks + 2, -1) # -1 means not scheduled yet
    Y_lower_bound = np.zeros(n_tasks + 2)  # Lower bound on arrival time at each
    ...
    # Task end times is (n_tasks), T_travel also includes start and end task (indexing +1)
    finish_task = n_tasks + 1
    last_depot_arrival_time = task_end_times[last_task_finished] + T_travel[last_task_finished+1][finish_task]
    print(task_end_times)
    print("\n\n", T_travel)
    print("Last finished task index", last_task_finished)
    print(T_travel[last_task_finished][n_tasks])
